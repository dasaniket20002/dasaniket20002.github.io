...
function Screen({ frame, panel, children, position, rotation, scale }: Screen) {
  ...
  return (
    <group position={position} rotation={rotation} scale={scale}>
      <mesh
        castShadow
        receiveShadow
        geometry={meshes[frame].geometry}
        material={materials.Texture}
      />
...
function ScreenText({ invert, x = 0, y = 1.2, ...props }: ScreenText) {
  ...
  useFrame((state) => {
    if (!textRef.current || !textRef.current.position || !randRef.current)
      return;
...
function SpinningBox({
  ...
  useFrame((_, delta) => {
    ...
    const target = clicked ? scale * 1.4 : scale * 1.2;

    // smooth easing toward target
    const current = ref.current.scale.x;
    const eased = MathUtils.damp(current, target, 6, delta);
...
function Leds({ sphere: Sphere }: { sphere: React.FC<InstanceProps> }) {
  ...
  useFrame((state) => {
    ...
    ref.current.children.forEach((child) => {
      const mesh = child as PositionMesh;

      const rand = Math.abs(2 + mesh.position.x);
      const t = (1 + Math.sin(rand * 10 + state.clock.elapsedTime * rand)) / 2;
  ...
  return (
    <group ref={ref}>
      <Sphere position={[-0.41, 1.1, -2.21]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[0.59, 1.32, -2.22]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[1.77, 1.91, -1.17]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[2.44, 1.1, -0.79]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[4.87, 3.8, -0.1]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[1.93, 3.8, -3.69]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[-2.35, 3.8, -3.48]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[-4.71, 4.59, -1.81]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[-3.03, 2.85, 1.19]} scale={0.005} color={[0, 2, 1]} />
      <Sphere position={[-1.21, 1.73, -1.49]} scale={0.005} color={[0, 2, 1]} />
    </group>
  );
}
